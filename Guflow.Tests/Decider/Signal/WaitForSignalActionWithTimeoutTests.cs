...
namespace Guflow.Tests.Decider
{
    ...
    public class WaitForSignalActionWithTimeoutTests
    {
        ...
        public void Returns_the_timer_decision_and_decision_to_wait_for_a_signal_when_the_lambda_completed_event_is_processed_immediately()
        {
            ...
            var graph = _graphBuilder.LambdaCompletedEventGraph(_confirmEmailId, "input", "result", completedStamp: completedStamp);
            ...
            decisions[0].AssertWaitForSignal(_confirmEmailId, lambdaCompletedEventId, SignalWaitType.Any, SignalNextAction.Continue,
                completedStamp,TimeSpan.FromHours(2),"Confirmed");
        ...
        public void Signal_is_ignored_when_it_come_after_wait_is_timedout_by_signal_timer()
        {
            ...
            var graph = _graphBuilder.LambdaCompletedEventGraph(_confirmEmailId, "input", "result", completedStamp: completionDate);
            _builder.AddProcessedEvents(graph);
            var completedEventId = graph.First().EventId;
            _builder.AddProcessedEvents(_graphBuilder.WaitForSignalEvent(_confirmEmailId, completedEventId, 
                                                                new[] { "Confirmed" }, SignalWaitType.Any, 
                                                                SignalNextAction.Continue, completionDate, TimeSpan.FromHours(2)));
        ...
        public void Wait_for_signal_is_timedout_when_timer_is_fired_before_receiving_the_signal()
        {
            ...
            var lg = _graphBuilder.LambdaCompletedEventGraph(_confirmEmailId, "i", "r", TimeSpan.FromSeconds(1), completedStamp);
            _builder.AddProcessedEvents(lg);
            var signalTriggerEventId = lg.First().EventId;
            _builder.AddProcessedEvents(_graphBuilder.WaitForSignalEvent(_confirmEmailId, signalTriggerEventId, new[] { "Confirmed" }
                                                                        , SignalWaitType.Any,SignalNextAction.Continue,
                                                                         completedStamp, TimeSpan.FromHours(2)));
        ...
        public void Signal_timer_is_ignored_when_it_is_fired_after_receiving_the_signal()
        {
            ...
            var signalTriggerEventId = lg.First().EventId;
            _builder.AddProcessedEvents(_graphBuilder.WaitForSignalEvent(_confirmEmailId, signalTriggerEventId, new[] { "Confirmed" }, 
                                        SignalWaitType.Any, SignalNextAction.Continue, completedStamp, TimeSpan.FromHours(2)));
        ...
        public void Return_timer_decision_and_continue_execution_with_signal_triggered_when_signal_and_lambda_completed_events_comes_together_and_signal_come_before_timeout()
        {
            ...
            decisions[0].AssertWaitForSignal(_confirmEmailId, triggerEventId, SignalWaitType.Any, SignalNextAction.Continue, 
                completedStamp,TimeSpan.FromHours(2), "Confirmed");
        ...
        public void Signal_timer_is_ignored_when_it_is_fired_after_receiving_the_signal_and_signal_timer_and_signal_are_processed_together_also_signal_was_received_after_timedout()
        {
            ...
            var signalTriggerEventId = lg.First().EventId;
            _builder.AddProcessedEvents(_graphBuilder.WaitForSignalEvent(_confirmEmailId, signalTriggerEventId, new[] { "Confirmed" }, SignalWaitType.Any,
                    SignalNextAction.Continue, completedStamp, TimeSpan.FromHours(2)));
